...
namespace
{
  ...
  filesystem8::directory_iterator end_dir_itr;
...
#   ifdef FILESYSTEM8_POSIX_API
    ...
    if (e1 != 0 || e2 != 0)
    {
      ...
      error (e1 != 0 && e2 != 0, p1, p2, ec, "filesystem8::equivalent");
      return false;
    }
...
#   else  // Windows
    ...
    if (error((fad.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)!= 0
      ? ERROR_NOT_SUPPORTED : 0, p, ec, "filesystem8::file_size"))
      return static_cast<std::uintmax_t>(-1);
...
#     endif
  ...
  path relative(const path& p, const path& base, error_code* ec)
  {
    ...
    if (error(tmp_ec.value(), base, ec, "filesystem8::relative"))
      return path();
...
#     endif
      ...
      {
        error(ENOTDIR, p, ec, "filesystem8::temp_directory_path");
        return p;
      }
...
#   else  // Windows
      ...
      if (p.empty())
      {
        ...
        {
          error(::GetLastError(), ec, "filesystem8::temp_directory_path");
          return path();
        }
  ...
  path weakly_canonical(const path& p, std::error_code* ec)
  {
    ...
    for (; !head.empty(); --itr)
    {
      ...
      if (error(head_status.type() == fs::status_error,
        head, ec, "filesystem8::weakly_canonical"))
        return path();
    ...
    if (error(tmp_ec.value(), head, ec, "filesystem8::weakly_canonical"))
      return path();
    return tail.empty()
      ? head
...
#   endif
    ...
    const path& p, std::error_code* ec)    
  {
    if (error(p.empty() ? not_found_error_code.value() : 0, p, ec,
              "filesystem8::directory_iterator::construct"))
      return;
...
#     endif
    ...
    if (result)
    {
      ...
      error(result.value(), p,
        ec, "filesystem8::directory_iterator::construct");
      return;
    }
